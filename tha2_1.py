...
# ???? Just returning Jacobian is okay? 
    ...
    def redundancy_resolution(robot_k, joint_angles, joint_loc, joint_axes, joint_type, v_e, omega, k_0):
        ...
        S, _ = robot_k.FK_spatial(end_effector, joint_loc, joint_axes, joint_type, joint_angles, display=False)
        Js = robot_k.J_space(S, joint_angles)
        
        # Calculate the pseudoinverse of the Jacobian
        pseudo_Jacobian = Js.T @ np.linalg.inv(Js @ Js.T)
        
        # Calculate the gradient of the omega function
        omega_gradient = np.gradient(omega(joint_angles))
        
        # Calculate q_dot_0
        q_dot_0 = k_0 * omega_gradient
        
        # Calculate q_dot
        I_n = np.eye(len(joint_angles))
        q_dot = pseudo_Jacobian @ v_e + (I_n - pseudo_Jacobian @ Js) @ q_dot_0
